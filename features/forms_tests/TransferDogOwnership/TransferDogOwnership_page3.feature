...
#    Then I click on "next" button
  ...
  Scenario: accepted check
    Given I navigate to "DogHoldingLic" page
    Then  check if area accepted check is available
    When click in accepted check "declaration1"
    And click in accepted check "declaration2"
    And click in accepted check "declaration3"
    Then check if accepted check "declaration1" selected
    Then check if accepted check "declaration2" selected
    Then check if accepted check "declaration3" selected
    When I write "ווסאם" in "אימות:"
